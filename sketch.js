...
function setup() {
  ...
  player = createSprite(40, displayHeight - 70);
...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    if (player.isTouching(pollutionGroup)&&seedCount>0&&keyDown("space")) {
      ...
      plant.scale = 0.3;
      plant.velocityX = -3;
      pollutionhealth=pollutionhealth-10;
    ...
    if (meteorGroup.isTouching(ground)||meteorGroup.isTouching(topGroundGroup) && playerhealth>0) {
    ...
    // console.log(gunCount);
      if (World.frameCount%10000==0&& count==0) {
      spawnMonster();
      }
 ...
 if (playerhealth == 0||pollutionhealth==0) {
    gameState = "end";
  }
...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    // // fightSprite.bounceOff(inGround);
    ...
    //    fightSprite.y -= 10;
    //  }
    //  fightSprite.y = fightSprite.y + 200;
 //fightSprite.collide(ground);
    ...
    if (fireballGroup.isTouching(player) && playerhealth > 0) {
      fireballGroup.destroyEach();
      playerhealth=playerhealth-15;
    }
    if (waterBalloonGroup.isTouching(fightSprite)) {
      pollutionhealth = pollutionhealth - 30;
         gameState = "play";
       fightSprite.destroy();
       waterBalloonGroup.destroyEach();
       fireballGroup.destroyEach();
  ...
  drawSprites();
   if (gameState == "serve") {
    ...
    text("EARTH WARS", displayWidth / 2, displayHeight / 2 - displayHeight / 2 + 60);
  

       textSize(30);
       fill(250);
       textStyle(BOLDITALIC);
    text(description, displayWidth / 2 - 250, displayHeight / 2-100);
}
  if (gameState == "play") {
    textSize(15);
    fill(250);
    ...
    textSize(40);
       if (World.frameCount>0&&World.frameCount<400) {
...
function spawnMeteor() {
  ...
  if (rand == 1) {
    ...
    meteor.velocityX = Math.round(random(6, 10));
    meteor.velocityY = Math.round(random(6, 10));
